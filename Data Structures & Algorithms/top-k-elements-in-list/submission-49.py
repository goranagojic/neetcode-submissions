class Solution:
    def _place_num_bucket(self, nums, buckets):
    ...
    def _place_num_bucket_hash(self, nums, buckets):
        frequencies = dict()
        for num in nums:
    ...
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        # This solution has
        # - time complexity O(N^2) due to _self.place_num_bucket implementation, with self._place_num_bucket_has it is O(N)
        # - memory complexity O(N) which is good
        buckets = [set() for _ in nums]

        # populate buckets
        # for num in nums:
        # self._place_num_bucket(nums, buckets)
        self._place_num_bucket_hash(nums, buckets)
        ...
        for bucket in reversed(buckets):    # elements toward the end are the most frequent, so we iterate from the end
            if len(bucket) == 0:
                continue

            for element in bucket:
                if k > 0:       # if element not already in results, add it
                    ...
                    break       # added k element, end and return result
