class Solution:
    ...
    def threeSumTry(self, nums: List[int]) -> List[List[int]]:
        nums.sort()
        l, r = 0, len(nums) - 1
        ...
        for l in range(0, len(nums)-1):
            for r in range(len(nums)-1, -1, -1):
                ...
                if melem < 0:
                    ...
                    m = r - 1
                    if m == l: # no remaining elements 
                        break
                    while m > l and melem < nums[m]: 
                        m -= 1
                    if melem == nums[m]:
                        results.add(tuple([lelem, relem, melem]))
    ...
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in range(0, len(nums)-2):
            ...
            while l < r:
                ...
                print(s) 
                if s == 0:
                    results.add(tuple([a, nums[l], nums[r]]))
                    l += 1
                    r += 1
                if s < 0:
                    l += 1
