    ...
    def fourSum(self, nums: List[int], target: int) -> List[List[int]]:
        ...
        for fi in range(0, len(nums)-3):
            ...
            for fj in range(fi+1, len(nums)-2):
                ...
                i = fj+1
                c = nums[i]
                if i > fj+1 and c == f[i-1]:        # skip checking the same value for i, already tested - boundary checks are important!
                    i += 1
                    continue

                j = len(nums)-1
                d = nums[j]
                if j < len(nums)-1 and d == f[j+1]: # skip checking the same value for j, already tested - boundary checks are important!
                    j -= 1
                    continue
