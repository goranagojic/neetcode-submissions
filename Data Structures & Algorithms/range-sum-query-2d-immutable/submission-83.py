    ...
    def __init__(self, matrix: List[List[int]]):
        ...
        for i in range(0, len(matrix)):
            for j in range(0, len(matrix[0])):
                prefix_side = self.matrix[i][j-1] if j > 0 else 0
                prefix_above = self.matrix[i-1][j] if i > 0 else 0
                prefix_diag = self.matrix[i-1][j-1] if i > 0 and j > 0 else 0
                self.matrix[i][j] = matrix[i][j] + prefix_above + prefix_side - prefix_diag
