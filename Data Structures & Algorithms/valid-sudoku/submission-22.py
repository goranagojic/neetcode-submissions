class Solution:
    ...
    class Sudoku:
        ...
        def _get_subboard(self, row_idx, n_rows, col_idx, n_cols):

            flattened_subboard = []
            for i in range(row_idx, row_idx+n_rows, 1):
                flattened_subboard.extend(self.board[i][col_idx:col_idx+n_cols])

                print(f"subboard: r {row_idx}, c {col_idx}, v: {self.board[i][col_idx:col_idx+n_cols]}")

            print(f"complete subboard: {flattened_subboard}")
            return flattened_subboard

        def has_duplicates_square(self, row_idx, n_rows, col_idx, n_cols):

            square_f = self._get_subboard(row_idx, n_rows, col_idx, n_cols)
            print(square_f)
